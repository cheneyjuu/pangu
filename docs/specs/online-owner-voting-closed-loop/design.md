...
- `acknowledge`：校验 C 端 L2 实名身份和冻结表决代表，写阅读确认及线上送达；
...
- 非选举共同决定必须为 `C_USER`、至少 L2 实名、当前租户和冻结表决代表；业委会选举线上投票由选举入口另行要求 L3；
...
- 集成测试覆盖 L1 被拒、L2 可办理非选举共同决定、冻结代表、阅读确认、事项全集、表决包摘要和跨渠道唯一；
- 事务测试覆盖某一事项冲突时整包零写入；
