...
- Yaochi 已增加物业侧完整邀价/报价/比价/修订/定商工作面，供应商工作台新增项目邀价入口；方案锁定只要求中选快照和现场照片，合同供应商改为只读继承中选结果。
- Shennong 业主端已披露中选施工单位、报价金额、报价说明、推荐依据和不足三家说明，并把中选报价原件纳入既有受控附件查看链路。
- 新方案版本在项目仍为 `PLAN_LOCKED` 时，供应商也能收到该草稿方案的新一轮邀价；对应后端回归已通过。
- 已修复供应商无单一租户身份的真实登录路径：后端以邀价记录解析项目租户，供应商工作台、附件上传/下载和在线报价不再依赖令牌中的 `tenant_id`；回归测试同步改用真实无租户令牌。
- 浏览器已完成物业端邀价、三家报价、比价定商的桌面/移动端闭环，以及供应商真实账号登录和工程邀价请求验收；相关接口返回 200，页面无控制台错误或横向溢出。
- 最终验证通过：Pangu 631 tests（0 failures、0 errors、1 skipped），Yaochi 生产构建，Shennong TypeScript 检查及正式微信小程序构建。
- 已提交 Pangu `f67ac7a`、`9c888c1`、`29617cb`，Yaochi `249d7ae`、`ac96339`，Shennong `daa7be2`。
