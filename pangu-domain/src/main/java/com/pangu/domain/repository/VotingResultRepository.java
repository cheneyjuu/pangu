 ...
 * application 层判断后调用 {@link #upsert} 自增 {@code settle_version}。
 *
...
public interface VotingResultRepository {
     ...
     *
     * <p>若已存在记录则递增 {@code settleVersion} 并替换 payload；通常应当在
     ...
     * @param settleVersion           结算版本（首次为 1，重新结算时由仓储递增）
    ...
    record Snapshot(
        ...
        public Snapshot {
            ...
            if (settleVersion < 1) {
                throw new IllegalArgumentException("settleVersion must be >= 1");
            }
            if (totalArea == null) {
