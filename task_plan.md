...
| 3 | 将 Yaochi 工程项目流程接入项目级供应商选择 | complete |
| 4 | 将中选报价纳入锁定方案和有权业主披露 | complete |
| 5 | 执行后端测试、前端构建、数据迁移与流程回归 | complete |
...
| 供应商真实工作身份不携带单一 `tenant_id`，但最初集成测试错误地为供应商令牌写入小区，掩盖了工作台 403 | 2 | 供应商测试统一使用真实无租户令牌；报价机会、附件和报价提交必须以服务端邀价记录反查租户，禁止前端传租户或使用默认小区 |
