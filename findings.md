# Findings: 维修工程项目询价、比价与定商闭环

- 共有部分工单勘验完成后强制转入工程项目台账，旧工单供应商流程只读或不再开放；因此不能把旧接口仍存在误判为新主流程已经覆盖报价。
- 新项目目前只保存选择方式和理由、上传一份无供应商归属的原始报价，并在治理通过后从任意已核验供应商中登记合同；报价、推荐、方案和合同之间没有引用链。
- 旧工单已经实现供应商邀请、修订报价、物业代录、供应商在线确认、至少三家竞争性询价、报价不足说明、框架关系校验和推荐快照，业务规则应迁移复用而不是重新发明。
- 一个维修工程项目可以合并多个报修工单，报价必须归属项目及具体方案版本，不能继续借用单一 `work_order_id` 作为报价聚合根。
- 新项目在 `DRAFT` 状态只能上传一份无归属报价并锁定方案，`AUTHORIZED` 后合同可从全部已核验供应商中任意选择；服务端只校验供应商资质和合同金额上限，没有校验中选报价。
- 旧工单的竞争性询价规则是“至少邀请 3 家；有效报价不足 3 家必须说明；框架方式必须关联有效长期合作关系”，这些是现有系统业务规则，不应在迁移时弱化。
- 项目方案创建后没有原地编辑接口，因此中选结果应作为绑定 `project_id + plan_id` 的独立不可变快照，在锁定方案时一并进入快照哈希，而不是回写已创建的方案正文。
- 旧报价附件引用工单附件，项目附件使用独立表和权限边界；直接给旧报价表增加项目外键会同时污染聚合主键、附件外键和唯一索引，项目级采购实体更符合现有边界。

## Historical Findings

# Findings: 维修实施方案编辑与业主端披露

- 当前项目表单声明 `max-w-5xl`，但通用 `DialogContent` 在桌面断点带有 `sm:max-w-lg`，响应式规则覆盖基础宽度，导致实际工作区约 512px；这类长表单应改为独立页面，而不是继续补弹窗宽度。
- 重构方案已经要求 C 端查看锁定实施方案、费用分摊、治理依据、正式表决和完工披露；当前 C 端只显示 `PROJECT_LINKED` 提示，并仅有受影响业主验收任务接口，尚无业主侧项目方案读模型。
- 项目方案现有模型已经版本化保存问题原因、实施范围、工程项、预算、分摊、供应商选择、施工/安全要求、验收规则、工期、质保和付款节点，适合生成业主只读投影，不需要把结构化事实重新塞进单一富文本。
- Yaochi 已有 Markdown 驱动的 `RichTextEditor` 和小程序预览转换，但其允许标签多于 Shennong；两端白名单不一致，而且清洗只发生在客户端。公开方案必须由后端统一清洗、规范化并纳入方案快照。
- 业主侧最小可信入口应通过已可见报修事项查找关联项目，再由后端校验项目已锁定及范围匹配；不能直接复用包含内部字段的管理端 `RepairProject.Details`。
