...
public class RepairProjectProcessHistoryService {
    ...
    private static final Map<String, HistoryDescriptor> BUSINESS_HISTORY = Map.ofEntries(
            ...
            Map.entry("RESPONSIBILITY_DETERMINATION_PROPOSED", new HistoryDescriptor("提交工程责任初判", "已提交责任和资金承担初判，等待有权主体确认。")),
            Map.entry("RESPONSIBILITY_DETERMINATION_CONFIRMED", new HistoryDescriptor("确认工程责任初判", "已确认本工程的责任路径和资金承担；共有维修仍须取得相关业主决定。")),
            Map.entry("AUTHORIZATION_PROPOSAL_FROZEN", new HistoryDescriptor("冻结相关业主决定提案", "已固定供相关业主决定审查的工程范围、预算和费用承担基数。")),
